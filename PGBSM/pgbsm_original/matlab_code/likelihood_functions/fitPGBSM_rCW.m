function [mle,LL,POST] = fitPGBSM_rCW(MLE_nullPGBM,CM,SEQI,TF,Y,pi_state,lambda,genetic_code)
...
disp('Fitting the rCW model ...')

% (piM3 w1CL w2CL p1CL delta kappa lambda pi_rCW B)
...
tic
f = @(first_guess)likelihoodFun_rCW(first_guess,CM,TF,SEQI,pi_state,Y,dMap,dP,genetic_code);
mle = fmincon(f,first_guess,A,B,Aeq,Beq,lb,ub,[],options);
toc

[LL,POST] = likelihoodFun_rCW(mle,CM,TF,SEQI,pi_state,Y,dMap,dP,genetic_code);
