function [mle,LL,POST] = fitPGBSM_BW(MLE_nullPGBM,CM,SEQI,TF,Y,pi_state,lambda,genetic_code)

% sample the history of the phenotype
[cMap,cP]= SampleAncestralPhenotypes(lambda,pi_state,CM,Y,1e5);
...
tic
f = @(first_guess)likelihoodFun_BW(first_guess,CM,TF,SEQI,pi_state,Y,cMap,cP,genetic_code);
mle = fmincon(f,first_guess,A,B,Aeq,Beq,lb,ub,[],options);
toc

[LL,POST] = likelihoodFun_BW(mle,CM,TF,SEQI,pi_state,Y,cMap,cP,genetic_code);
